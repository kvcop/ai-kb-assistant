...
class TestWatchRemindersTopicDelivery(unittest.TestCase):
    def test_sends_reminders_to_configured_topic(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            st.load()
            st.set_reminders_target(chat_id=111, message_thread_id=777)
    ...
    def test_allows_group_chat_id_as_default_target(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            reminders = root / 'reminders.md'
            reminders.write_text('date:2025-12-28@12:00\tTest reminder\n', encoding='utf-8')
            ...
            self.assertEqual(len(api.sent), 1)
            chat_id, thread_id, _text = api.sent[0]
